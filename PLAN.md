...
## Art renderer (Phase 2, built)
- **Pattern fills** (`src/engine/patterns.ts`): hatch, crosshatch, lines, dots, stipple and waves, generated on demand
  for water, parks, forests and buildings via a style's `patterns` block.
- **Effects pass** (`src/engine/effects.ts`): one WebGL shader composites map + text and applies wobble,
  misregistration, paper mottling, grain, pixelation and vignette. Configured by a style's `effects` block.
  Sizes are in poster-width units, so the preview and 300 DPI exports match. Users get an on/off switch and strength slider.
...
Status key: ✅ done · 🚧 flat version done; full look needs a later engine phase · ⬜ planned.
